...
class TransformezHook(FetchHook):
    ...
    1. Stage 'pre': Generates the master shift grid using module.region.
    2. Stage 'file': Applies the shift grid to each downloaded file.
    ...
    #         return self._run_pre(entries)

    #     return self._run_file(entries)

    
    #def _run_pre(self, entries):
    def run(self, entries):
    ...
    def _run_file(self, entries):
        ...
        for mod, entry in entries:
            ...
            if ext in ['.tif', '.gtx'] and self.has_gdal:
                transformed_path = self._apply_raster(filepath, self.output_grid)
            elif ext in ['.laz', '.las'] and self.has_pdal:
